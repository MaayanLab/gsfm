...
class RummageneDataModule(L.LightningDataModule):
  def __init__(self, batch_size: int = 64, max_size: int = 1000):
    super().__init__()
    ...
    if stage == 'fit':
      if getattr(self, 'rummagene_train', None) is None:
        self.rummagene_train = [
          clean_genes
          for _, year, genes in read_gmt(f'data/2025-08-29-rummagene.gmt')
          for clean_genes in (list(set(map(str.upper, filter(None, genes)))),)
          if len(clean_genes) > 10 and len(clean_genes) < self.max_size
        ]
