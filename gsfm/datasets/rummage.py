...
class GeneSetDataModule(L.LightningDataModule):
  ...
  def setup(self, stage: str):
    ...
    if stage == 'fit':
      self.rummage_train = [
        clean_genes
        for it in [
          utils.read_gmt('data/2025-08-29-rummageo-mouse.gmt'),
    ...
    @utils.if_not_exists('data/2025-08-29-rummage-vocab.pickle')
    def _(filename):
    ...
    self.vocab = utils.Vocab.from_file('data/2025-08-29-rummage-vocab.pickle')
  ...
  #   return torch.utils.data.DataLoader(self.rummage_val, batch_size=256, collate_fn=GeneSetDataModule._collate_fn, shuffle=True)
